...
public class RoomController {
    ...
    @PostMapping("/create")
    public ResponseEntity<RoomResponseDto> groupCreation(
    ...
    public ResponseEntity<CommonAPIMessage> outOfTheRoom(
            @PathVariable(value = "roomId") String roomId, @RequestParam Long uid) {
        if (roomService.exitRoom(roomId, uid)) {
            return new ResponseEntity<>(
                    new CommonAPIMessage(
                            CommonAPIMessage.ResultEnum.success,
        ...
        return new ResponseEntity<>(
                new CommonAPIMessage(
                        CommonAPIMessage.ResultEnum.failed,
                        new HashMap<String, String>() {
                            {
    ...
    @GetMapping("/{roomId}")
    public ResponseEntity<RoomResponseDto> chatRoomInfo(
    ...
    @Operation(
            summary = "채팅방 리스트 조회",
            description = "특정 유저가 참여중인 채팅방 리스트 조회",
            responses = {
                @ApiResponse(
                        responseCode = "200",
                        description = "조회 성공",
    ...
    @Operation(
            summary = "채팅방 리스트 조회",
            description = "특정 유저가 참여할 수 있는 채팅방 리스트 조회",
            responses = {
                @ApiResponse(
                        responseCode = "200",
                        description = "조회 성공",
