...
public interface UserRepository extends Neo4jRepository<User, Long> {
    ...
    @Query(value = "MATCH (a:User) WHERE a.userId = $userId RETURN a")
    ...
    @Query(
            value =
                    "MATCH (a:User{userId:$fromUserId}), (b:User{userId:$toUserId}) RETURN EXISTS((a)-[:IS_FOLLOWING]->(b))")
    boolean isFollowing(String fromUserId, String toUserId);

    @Query(
            value =
                    "MATCH (a:User)-[r:IS_FOLLOWING]->(b:User) WHERE a.userId = $fromUserId AND b.userId = $toUserId DELETE r")
